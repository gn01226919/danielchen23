<!-- BEGIN:nextjs-claude-rules -->
# This is NOT the Next.js you know

This version has breaking changes — APIs, conventions, and file structure may all differ from your training data. Read the relevant guide in `node_modules/next/dist/docs/` before writing any code. Heed deprecation notices.
<!-- END:nextjs-claude-rules -->

---

# danielchen23 · 專案 Claude 規則

> 本檔由 `AGENTS.md` 複製並改寫，供 **Claude Code / Claude** 讀取。  
> 資安條款與 `AGENTS.md` **同等效力**；若兩檔衝突以較嚴為準。  
> 全域規則另見：`~/.claude/Claude.md`（操作安全、繁中回覆、TBI 等）。
...
| **主站** | **科技深色 UI** `/`（`src/app/(site)/`） |
| 風格預覽 | `/v` 混血 · 編輯室 · 紙感；`/v/tech` redirect 到 `/` |
...
- ❌ Webhook 不驗簽就改訂閱狀態  
- ❌ Commit `.env.local` 或把 key 寫進 `CLAUDE.md` / `AGENTS.md` / README  
...
6. 遵守全域 `~/.claude/Claude.md`：改檔前說明、繁中回覆、不擅自刪檔、不自動裝套件（先確認）。  
7. 本專案對話結束前：`git commit` + `push`；並視需要更新 Obsidian `projects/danielchen23 brand/`。
...
| Supabase schema | `supabase/schema.sql`（擴充時必加 RLS） |
| 主站（科技） | `src/app/(site)/` |
| 科技 CSS | `src/app/v/tech/tech.css` |
| 後台 | `src/app/admin/` |
| 會員／鎖文 | `src/lib/server/membership.ts` |
| 設定指南 | `docs/SETUP-AUTH-STRIPE-EMAIL.md` · `docs/SUPABASE-PROJECT.md` |
| 本檔（Claude） | `CLAUDE.md` |
| 同源規則（通用 Agent） | `AGENTS.md` |
...
| 項目 | 路徑 |
|------|------|
| 筆記資料夾 | `~/Library/Mobile Documents/iCloud~md~obsidian/Documents/Vault/projects/danielchen23 brand/` |
| 進度 | `status.md` |
| 日誌 | `開發日誌.md` |
| 待辦 | `待辦事項.md` |
